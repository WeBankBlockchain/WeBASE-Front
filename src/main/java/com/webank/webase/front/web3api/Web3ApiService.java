...
//                    .send().getTransactionCount();
    ...
    public Object searchByCriteria(int groupId, String input) {
        ...
        if (input.matches("^[a-zA-Z]*")) {
            String contractName = input.split(" ")[0];
            String version = input.split(" ")[1];
            return contractService.findByCriteria(groupId, contractName, version, null);
        } else if (input.startsWith("0x")) {
            if (input.length() == 42) {
                return transLogService.findByCriteria(groupId, null, null, input);
