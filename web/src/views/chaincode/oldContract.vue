        ...
        <el-table :data="contractList" tooltip-effect="dark" v-loading="loading">
          ...
          <el-table-column :label="$t('table.actions')" width="280">
            <template slot-scope="scope">
              ...
              <el-button v-show="!liquidCheck" :disabled="!scope.row.contractAddress || !scope.row.haveEvent" :class="{'grayColor': !scope.row.contractAddress || !scope.row.haveEvent}" @click="handleEvent(scope.row)"
                type="text" size="small">{{$t('title.checkEvent')}}</el-button>
...
import { getContractList, liquidCheck, checkIsWasm } from "@/util/api";
